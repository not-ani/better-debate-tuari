import { addRootFromDialog, open } from "./electrobun/shims/dialog";
import { openPath } from "./electrobun/shims/opener";
import { listen, type UnlistenFn } from "./electrobun/shims/event";
import {
  checkForUpdates,
  getUpdateStatus,
  installUpdateNow,
  type DesktopUpdateStatus,
} from "./electrobun/shims/update";
import { Show, batch, createEffect, createMemo, createSignal, onCleanup, onMount, startTransition, untrack } from "solid-js";
import AppWorkspace from "./components/AppWorkspace";
import IndexesPage from "./components/IndexesPage";
import { ALL_ROOTS_KEY, TREE_OVERSCAN_ROWS, TREE_ROW_STRIDE_PX } from "./lib/constants";
import {
  getPersistedCaptureTargetForRoot,
  loadStoredBoolean,
  normalizeCaptureTargetPath,
  persistBooleanSetting,
  persistCaptureTargetForRoot,
  SEARCH_DEBATIFY_ENABLED_PREFS_KEY,
  SEARCH_FILENAME_ONLY_PREFS_KEY,
} from "./lib/app/settings";
import { reconcileTreeRows } from "./lib/app/treeReconcile";
...
function App() {
  ...
  const [rootIndexes, setRootIndexes] = createSignal<RootIndexEntry[]>([]);
  const [selectedRootPath, setSelectedRootPath] = createSignal("");
  const [currentPage, setCurrentPage] = createSignal<"workspace" | "indexes">("workspace");
  ...
  const [desktopUpdateStatus, setDesktopUpdateStatus] = createSignal<DesktopUpdateStatus | null>(null);
  ...
  const SEARCH_HYDRATE_OVERSCAN_ROWS = 12;
  const MAX_HYDRATED_SEARCH_RESULTS = 80;
  ...
  let stopUpdateStatusListener: UnlistenFn | null = null;
  ...
  const hydratedSearchResultsById = new Map<number, HydratedSearchResult>();
  const hydratedSearchLoadsInFlight = new Map<number, Promise<HydratedSearchResult | null>>();
  ...
  const resetPreviewCacheState = () => {
    ...
    hydratedSearchResultsById.clear();
    hydratedSearchLoadsInFlight.clear();
  ...
  const renderInlineFormattedHtml = (
    text: string,
    spans: Array<{ start: number; end: number; kind: string; color?: string | null }>,
  ) => {
    if (spans.length === 0) {
      return escapeHtml(text);
    }

    const charPositions = Array.from(text);
    const activeByStart = new Map<number, typeof spans>();
    const activeByEnd = new Map<number, typeof spans>();
    for (const span of spans) {
      const start = Math.max(0, Math.min(charPositions.length, span.start));
      const end = Math.max(start, Math.min(charPositions.length, span.end));
      if (start === end) continue;
      activeByStart.set(start, [...(activeByStart.get(start) ?? []), { ...span, start, end }]);
      activeByEnd.set(end, [...(activeByEnd.get(end) ?? []), { ...span, start, end }]);
    }

    const openTag = (span: { kind: string; color?: string | null }) => {
      if (span.kind === "highlight") {
        const colorClass = span.color ? ` bf-hl-${escapeHtml(span.color)}` : "";
        return `<mark class="bf-run bf-run-highlight${colorClass}">`;
      }
      if (span.kind === "underline") {
        return `<span class="bf-run bf-run-underline">`;
      }
      if (span.kind === "bold") {
        return `<strong class="bf-run bf-run-bold">`;
      }
      return `<span class="bf-run">`;
    };

    const closeTag = (span: { kind: string }) => {
      if (span.kind === "bold") return `</strong>`;
      return `</span>`;
    };

    let html = "";
    const stack: Array<{ start: number; end: number; kind: string; color?: string | null }> = [];
    for (let index = 0; index <= charPositions.length; index += 1) {
      const closing = (activeByEnd.get(index) ?? []).sort((left, right) => right.start - left.start);
      if (closing.length > 0) {
        for (const span of closing) {
          while (stack.length > 0) {
            const last = stack.pop()!;
            html += closeTag(last);
            if (last.start === span.start && last.end === span.end && last.kind === span.kind) {
              break;
            }
          }
          for (const reopen of stack) {
            html += openTag(reopen);
          }
        }
      }

      if (index === charPositions.length) break;

      const opening = (activeByStart.get(index) ?? []).sort((left, right) => right.end - left.end);
      for (const span of opening) {
        stack.push(span);
        html += openTag(span);
      }

      html += escapeHtml(charPositions[index]);
    }
  ...
  const outlineTrailText = (entries: Array<{ text: string }>) => entries.map((entry) => entry.text).join(" / ");

  const renderHydratedSearchPreview = (payload: HydratedSearchResult): SidePreview => {
    if (payload.entityType === "doc") {
      const headings = payload.headings
        .map((heading) => `<p class="bf-preview-p">${escapeHtml(`H${heading.level} ${heading.text}`)}</p>`)
        .join("");
      return {
        title: payload.fileName,
        subTitle: payload.relativePath,
        text: payload.headings.map((heading) => `H${heading.level} ${heading.text}`).join("\n"),
        kind: "heading",
        richHtml:
          `<p class="bf-preview-p">${escapeHtml(payload.absolutePath)}</p>` +
          `<p class="bf-preview-p">${escapeHtml(payload.relativePath)}</p>` +
          headings,
      };
    }

    const trail = outlineTrailText(payload.headingTrail);
    const body = payload.body
      .map(
        (paragraph) =>
          `<p class="bf-preview-p">${renderInlineFormattedHtml(paragraph.text, paragraph.spans)}</p>`,
      )
      .join("");
    const tagSub = payload.tagSub.trim()
      ? `<p class="bf-preview-p">${escapeHtml(payload.tagSub)}</p>`
      : "";
    const citeDate = payload.citeDate ? ` · ${escapeHtml(payload.citeDate)}` : "";

    return {
      title: payload.tag,
      subTitle: payload.relativePath,
      text: [payload.tag, payload.cite, ...payload.body.map((paragraph) => paragraph.text)].join("\n\n"),
      headingLevel: payload.headingLevel,
      kind: "heading",
      richHtml:
        `<p class="bf-preview-p">${escapeHtml(payload.absolutePath)}</p>` +
        (trail ? `<p class="bf-preview-p">${escapeHtml(trail)}</p>` : "") +
        `<p class="bf-preview-h4">${escapeHtml(payload.tag)}</p>` +
        tagSub +
        `<p class="bf-preview-p"><strong>${escapeHtml(payload.cite)}</strong>${citeDate}</p>` +
        body,
    };
  };

  const hydrateSearchResults = async (resultIds: number[]) => {
    const touchHydratedResult = (result: HydratedSearchResult) => {
      hydratedSearchResultsById.delete(result.resultId);
      hydratedSearchResultsById.set(result.resultId, result);
      while (hydratedSearchResultsById.size > MAX_HYDRATED_SEARCH_RESULTS) {
        const oldest = hydratedSearchResultsById.keys().next().value;
        if (oldest === undefined) break;
        hydratedSearchResultsById.delete(oldest);
      }
    };

    const missing = resultIds.filter(
      (resultId) => !hydratedSearchResultsById.has(resultId) && !hydratedSearchLoadsInFlight.has(resultId),
    );
    if (missing.length === 0) {
      const cached = resultIds
        .map((resultId) => hydratedSearchResultsById.get(resultId) ?? null)
        .filter((value): value is HydratedSearchResult => Boolean(value));
      cached.forEach(touchHydratedResult);
      return cached;
    }

    const pending = invokeTyped<SearchHydrateResponse>("hydrate_search_results", {
      resultIds: missing,
    }).then((response) => {
      response.results.forEach((result) => {
        touchHydratedResult(result);
        hydratedSearchLoadsInFlight.delete(result.resultId);
      });
      return response.results;
    });

    missing.forEach((resultId) => {
      hydratedSearchLoadsInFlight.set(
        resultId,
        pending.then(() => hydratedSearchResultsById.get(resultId) ?? null),
      );
    });
  ...
  const ensureSearchResultHydrated = async (resultId: number) => {
    const cached = hydratedSearchResultsById.get(resultId);
    if (cached) {
      hydratedSearchResultsById.delete(resultId);
      hydratedSearchResultsById.set(resultId, cached);
      return cached;
    }

    const inFlight = hydratedSearchLoadsInFlight.get(resultId);
    if (inFlight) return inFlight;

    await hydrateSearchResults([resultId]);
    return hydratedSearchResultsById.get(resultId) ?? null;
  };
  ...
  const refreshUpdateStatus = async () => {
    try {
      ...
      // Ignore missing updater status in dev and keep UI clean.
    }
  };
  ...
  const runUpdateCheck = async () => {
    ...
    try {
      ...
      if (nextStatus.error) {
        setStatus(`Update check failed: ${nextStatus.error}`);
      } else if (nextStatus.updateReady) {
        setStatus(`Update ready: ${nextStatus.info?.version ?? "latest"}`);
      } else if (nextStatus.info?.updateAvailable) {
        setStatus(`Update available: ${nextStatus.info.version || "latest"}`);
      } else {
  ...
  const runInstallUpdate = async () => {
    ...
    try {
      ...
      if (result.applied) {
        setStatus("Installing downloaded update and restarting...");
      } else {
        setStatus("Update install cancelled.");
      }
    } catch (error) {
  ...
  const openIndexesPage = () => {
    if (selectedRootPath() === ALL_ROOTS_KEY) {
      const firstRoot = roots()[0]?.path ?? "";
      if (firstRoot) {
        setSelectedRootPath(firstRoot);
      }
    }
    setStatus("Opening index manager...");
    setCurrentPage("indexes");
  };
  ...
  const loadRootIndexes = async () => {
    const entries = await invokeTyped<RootIndexEntry[]>("list_root_indexes");
    setRootIndexes(entries);
  };

  const loadRoots = async () => {
    const loaded = await invokeTyped<RootSummary[]>("list_roots");
    const loadedIndexes = await invokeTyped<RootIndexEntry[]>("list_root_indexes");
    const selected = selectedRootPath();
    const canKeepAllSelection = selected === ALL_ROOTS_KEY && loaded.length > 1;

    batch(() => {
      setRoots(loaded);
      setRootIndexes(loadedIndexes);
  ...
  const addFolder = async () => {
    ...
    try {
      const beforeRoots = await invokeTyped<RootSummary[]>("list_roots");
      const beforePaths = new Set(beforeRoots.map((root) => root.path));

      const dialogResult = await addRootFromDialog();
      const rootsAfter =
        dialogResult.rootsAfter.length > 0
          ? dialogResult.rootsAfter
          : await invokeTyped<RootSummary[]>("list_roots");
      ...
      const newestRoot = rootsAfter.find((root) => !beforePaths.has(root.path)) ?? rootsAfter[0] ?? null;
      const canonicalPath = dialogResult.canonicalPath?.trim() || newestRoot?.path || "";

      if (!canonicalPath) {
        setStatus("Folder selection cancelled or root add failed.");
        return;
      }

      setStatus(`Adding folder: ${canonicalPath}`);
  ...
  const reindexSelectedRoot = async () => {
    const rootPath = selectedRootPath();
    if (!rootPath || rootPath === ALL_ROOTS_KEY) {
      setStatus("Select a specific folder before reindexing.");
      return;
    }
    await runIndex(rootPath);
  };

  const openIndexPath = async (path: string) => {
    await openPath(path);
  };

  const deleteRootIndex = async (rootPath: string) => {
    if (!rootPath) return;
    if (isIndexing()) {
      setStatus("Wait for current indexing to finish before deleting an index.");
      return;
    }

    setStatus(`Deleting index: ${rootPath}`);
    try {
      await invokeTyped<null>("remove_root", { path: rootPath });
      await loadRoots();
      setStatus(`Deleted local index: ${rootPath}`);
    } catch (error) {
      setStatus(`Could not delete index: ${String(error)}`);
    }
  };
  ...
  const applyPreviewFromRow = (row: TreeRow) => {
    if (searchMode() && row.searchResult) {
      ...
      setSidePreview({
        title: result.headingText || result.fileName,
        subTitle: result.relativePath,
        text: [result.headingText, result.cite, result.snippet].filter(Boolean).join("\n\n"),
        headingLevel: result.headingLevel ?? null,
        kind: result.kind === "file" ? "heading" : "heading",
      });

      void ensureSearchResultHydrated(result.resultId).then((payload) => {
        if (!payload) return;
        setSidePreview(renderHydratedSearchPreview(payload));
      });
      return;
    }
  ...
  const openSearchResult = async (result: SearchHit) => {
    ...
    const hydrated = await ensureSearchResultHydrated(result.resultId);
    if (hydrated) {
      setSidePreview(renderHydratedSearchPreview(hydrated));
    }
  ...
  createEffect(() => {
    ...
    const timer = setTimeout(() => {
      const invocation = invokeTyped<SearchResponse>("search", {
        query,
        mode: semanticEnabled ? "mixed" : "keyword",
        rootPaths: searchRootPath ? [searchRootPath] : undefined,
        limit: 120,
        filters: {
          fileNameOnly,
        },
        ...
        .catch((error) => {
          ...
          return { results: [], warnings: [] } as SearchResponse;
        })
        .then((response) => {
          if (requestId === searchRequestSeq) {
            if (response.warnings.length > 0) {
              setStatus(response.warnings[0].message);
            }
            startTransition(() => {
  ...
  createEffect(() => {
    ...
    const rows = treeRows();
    const { start, end } = virtualWindow();
    const hydratedWindow = rows.slice(
      Math.max(0, start - SEARCH_HYDRATE_OVERSCAN_ROWS),
      Math.min(rows.length, end + SEARCH_HYDRATE_OVERSCAN_ROWS),
    );
    const visibleIds = hydratedWindow
      .map((row) => row.searchResult?.resultId ?? null)
      .filter((resultId): resultId is number => resultId !== null);
    if (visibleIds.length === 0) return;
    void hydrateSearchResults(Array.from(new Set(visibleIds)));
  });
  ...
  onMount(() => {
    ...
    let updateListenerDisposed = false;

    void refreshUpdateStatus();

    void listen<DesktopUpdateStatus>("update-status", (event) => {
      setDesktopUpdateStatus(event.payload);
    })
      .then((unlisten) => {
        if (updateListenerDisposed) {
          unlisten();
          return;
        }
        stopUpdateStatusListener = unlisten;
      })
      .catch(() => {
        // Ignore updater listener setup errors in environments without updater state.
      });
  ...
  return (
    <Show
      fallback={
        <AppWorkspace
          ...
          previewPanelWidthPx={previewPanelWidthPx}
          roots={roots}
          ...
          startLeftRailResize={startLeftRailResize}
          startPreviewPanelResize={startPreviewPanelResize}
          updateStatus={desktopUpdateStatus}
          isCheckingUpdates={isCheckingUpdates}
          isInstallingUpdate={isInstallingUpdate}
          checkForUpdates={runUpdateCheck}
          installUpdateNow={runInstallUpdate}
          openPath={openIndexPath}
          status={status}
    ...
    >
      <IndexesPage
        ...
        isIndexing={isIndexing}
        openPath={openIndexPath}
        openWorkspace={openWorkspacePage}
        refreshIndexes={loadRootIndexes}
        reindexSelected={reindexSelectedRoot}
        deleteIndex={deleteRootIndex}
