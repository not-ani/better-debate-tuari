...
use crate::types::{
    HeadingRange, ParsedCard, ParsedCardParagraph, ParsedHeading, ParsedParagraph, SearchInlineSpan,
};
...
pub(crate) fn run_style_id(run: Node<'_, '_>) -> Option<String> {
...
fn is_card_tag_style(style_label: &str) -> bool {
    normalize_for_search(style_label).contains("heading 4")
}

fn is_card_body_style(style_label: &str) -> bool {
    let normalized = normalize_for_search(style_label);
    normalized.contains("normal") || normalized.contains("list paragraph")
}

fn highlighted_run_text(paragraph: Node<'_, '_>) -> String {
    let mut output = Vec::new();
    for run in paragraph.children().filter(|node| has_tag(*node, "r")) {
        if run_highlight_class(run).is_some() {
            let value = extract_paragraph_text(run);
...
fn extract_run_text(run: Node<'_, '_>) -> String {
    extract_paragraph_text(run)
}

fn paragraph_spans(paragraph: Node<'_, '_>) -> Vec<SearchInlineSpan> {
    ...
    for run in paragraph.children().filter(|node| has_tag(*node, "r")) {
        ...
        if let Some(color) = run_highlight_class(run) {
            spans.push(SearchInlineSpan {
                start,
                end,
...
fn tokenize_cite_for_date(cite: &str) -> Vec<String> {
    let mut words = Vec::new();
    for raw in cite.split_whitespace() {
        let cleaned = raw
            .chars()
            .filter(|character| {
        ...
        if lowered == "and" || lowered == "or" || lowered == "of" {
            continue;
        }
        words.push(cleaned);
    }
...
fn generate_cite_date(cite: &str) -> Option<String> {
    let words = tokenize_cite_for_date(cite);
    if words.is_empty() {
        return None;
    }
    ...
    for start in 0..words.len() {
        ...
        for end in start + 1..=usize::min(words.len(), start + 4) {
            let mut combo = words[start..end].to_vec();
            let mut weight = 0_usize;
            let single_value_combo = combo.len() == 1;

            for item in &mut combo {
                weight += item.len();
                if (item.len() == 1 || item.len() == 2)
                    && item.chars().all(|c| c.is_ascii_digit())
                    && single_value_combo
                {
                    *item = append_to_year_string(item);
                    d_str = Some(item.clone());
                    weight += item.len();
                }
            }
            ...
            if combo
                .iter()
                .all(|item| !item.chars().all(|c| c.is_ascii_digit()) || item.len() < 4)
            {
                if let Some(year) = d_str.as_ref() {
            ...
            if let Some(date) = parse_date_candidate(&joined).filter(|date| date.year() > 1900) {
                if best
                    .as_ref()
                    .map(|(best_weight, _)| weight > *best_weight)
                    .unwrap_or(true)
                {
                    best = Some((weight, date));
                }
            }
        }
    }

    best.map(|(_, date)| date.format("%Y-%m-%d").to_string())
}
...
pub(crate) fn parse_docx_cards(file_path: &Path) -> CommandResult<Vec<ParsedCard>> {
    let file = File::open(file_path)
        .map_err(|error| format!("Could not open '{}': {error}", path_display(file_path)))?;
    let mut archive = ZipArchive::new(file)
        .map_err(|error| format!("Could not read '{}': {error}", path_display(file_path)))?;
    ...
    struct CardParagraph {
        order: i64,
        text: String,
        style_label: Option<String>,
        highlighted_text: String,
        spans: Vec<SearchInlineSpan>,
    }

    let mut paragraphs = Vec::<CardParagraph>::new();
    let mut order = 0_i64;
    for paragraph in document.descendants().filter(|node| has_tag(*node, "p")) {
        order += 1;
        paragraphs.push(CardParagraph {
            ...
            style_label: paragraph_style_label(paragraph, &style_map),
            highlighted_text: highlighted_run_text(paragraph),
            spans: paragraph_spans(paragraph),
        });
    }
    ...
    while cursor < paragraphs.len() {
        ...
        let is_tag = paragraph
            .style_label
            .as_deref()
            .map(is_card_tag_style)
            .unwrap_or(false);
        if !is_tag {
            cursor += 1;
            continue;
        ...
        let tag = paragraph.text.trim().trim_matches(',').trim().to_string();
        let heading_order = paragraph.order;
        cursor += 1;

        let mut cite = String::new();
        let mut tag_sub = Vec::new();
        let mut body = Vec::new();
        let mut highlighted = Vec::new();
        let mut found_cite = false;

        while cursor < paragraphs.len() {
            let candidate = &paragraphs[cursor];
            let next_is_tag = candidate
                .style_label
                .as_deref()
                .map(is_card_tag_style)
                .unwrap_or(false);
            ...
            if !text.is_empty() {
                if !found_cite && text.chars().any(|character| character.is_ascii_digit()) {
                    cite = text.to_string();
                    found_cite = true;
                } else if !found_cite {
                    tag_sub.push(text.to_string());
                } else if found_cite
                    && candidate
                        .style_label
                        .as_deref()
                        .map(is_card_body_style)
                        .unwrap_or(true)
                {
                    body.push(ParsedCardParagraph {
                        text: text.to_string(),
                        spans: candidate.spans.clone(),
                    });
                    if !candidate.highlighted_text.trim().is_empty() {
                        highlighted.push(candidate.highlighted_text.trim().to_string());
                    }
                }
            }
        ...
        let body_text = body
            ...
            .collect::<Vec<_>>()
            .join(" ");
        if body_text.trim().chars().count() < 25 {
            continue;
        }

        let cite_date = generate_cite_date(&cite);
        cards.push(ParsedCard {
            tag,
            tag_sub: tag_sub.join("\n"),
            cite,
            cite_date,
            body,
            highlighted_text: highlighted.join(" "),
            heading_order,
            heading_level: 4,
        });
    }
