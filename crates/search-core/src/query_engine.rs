...
const ROOT_ID_CACHE_TTL_MS: i64 = 300_000;
...
static ROOT_ID_CACHE: std::sync::OnceLock<std::sync::Mutex<HashMap<String, RootIdCacheEntry>>> =
...
fn root_id_cache() -> &'static std::sync::Mutex<HashMap<String, RootIdCacheEntry>> {
    ROOT_ID_CACHE.get_or_init(|| std::sync::Mutex::new(HashMap::new()))
}

pub(crate) fn set_cached_root_id(root_path: String, root_id: Option<i64>) {
...
pub(crate) fn invalidate_cached_root_id(root_path: &str) {
    if let Ok(mut cache) = root_id_cache().lock() {
...
fn resolve_requested_root_id(
    ...
    if let Ok(mut cache) = root_id_cache().lock() {
        if let Some(cached) = cache.get(&canonical).copied() {
            if now_ms() - cached.created_at_ms <= ROOT_ID_CACHE_TTL_MS {
                return Ok(cached.root_id);
            }
    ...
    set_cached_root_id(canonical, resolved);
    Ok(resolved)
}
...
pub(crate) async fn search_semantic(
    ...
    let requested_root_id = resolve_requested_root_id(app, root_path)?;
    vector::trigger_rebuild(app.clone(), requested_root_id, false);
    vector::search(
        app,
        cleaned_query,
        requested_root_id,
