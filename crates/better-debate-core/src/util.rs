...
pub(crate) const INDEX_LOG_HEARTBEAT_INTERVAL_MS: i64 = 2_000;
    ...
    std::thread::available_parallelism()
        .map(|parallelism| {
            ...
            if threads <= 4 {
                ...
                threads.saturating_mul(2).saturating_sub(2).max(8)
            }
        })
        .unwrap_or(4)
        .clamp(4, 32)
...
#[derive(Clone)]
pub(crate) struct IndexRunLogger {
    path: PathBuf,
}
...
impl IndexRunLogger {
    pub(crate) fn create(
        ...
        let logs_dir = app
            .path()
            .app_data_dir()
            .map_err(|error| format!("Could not resolve app data dir for index logs: {error}"))?
            .join("logs")
            .join("indexing");
        fs::create_dir_all(&logs_dir).map_err(|error| {
            format!(
                "Could not create index log dir '{}': {error}",
                path_display(&logs_dir)
            )
        })?;
        ...
        let logger = Self { path: log_file };
        logger.append(
            "INFO",
            "boot",
            &format!("index log created root=\"{}\"", root_path),
        )?;
        Ok(logger)
    }
    ...
    pub(crate) fn append(&self, level: &str, phase: &str, message: &str) -> CommandResult<()> {
        let mut file = OpenOptions::new()
            ...
            .map_err(|error| {
                format!(
                    "Could not open index log '{}': {error}",
                    path_display(&self.path)
                )
            })?;
        ...
        .map_err(|error| {
            format!(
                "Could not write index log '{}': {error}",
                path_display(&self.path)
            )
        })?;
        file.flush().map_err(|error| {
            format!(
                "Could not flush index log '{}': {error}",
                path_display(&self.path)
            )
        })?;
        file.sync_data().map_err(|error| {
            format!(
                "Could not sync index log '{}': {error}",
                path_display(&self.path)
            )
        })?;
        Ok(())
